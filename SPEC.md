...
<h2 style="font-size: 1.5em; font-weight: normal; color: #555; margin-top: 0;">Technical Specification</h2>  <p style="margin-top: 4em; font-size: 1.1em;">        <strong>Version:</strong> 1.0.86</p>
...
|---------|------|------------|
| v1.0.86 | 2026-09-08 | Tool-calling audit remediation (shipped in v1.0.86): fixes every HIGH/MED finding from the tool-calling + UTF-8 audit across provider stream parsers, tool dispatch, and the edit-tool family. `openai_responses` emits `ToolCallStart` from `response.output_item.added` (Responses-API tool calls were silently dropped); Gemini parses parts BEFORE the finishReason flush plus an end-of-stream flush with an emitted-id set (final-chunk `functionCall` was lost); object-form `arguments` accepted in openai/ollama/copilot/huggingface/openrouter (llama.cpp/vLLM style); anthropic + azure_resource key open blocks by SSE block index (BTreeMap; azure never accumulated args before); ollama/ollama_cloud stream-scoped narration suppression; router `select_tier_entry` gained `needs_tools` (tool-incapable entries skipped with fallback + warn, HuggingFace gated on declared capability); processor `parsed_input: Result<Value, String>` single-parse replaces 7 silent `json!({})` sites — parse failure short-circuits with a corrective LLM-visible error; loop restriction fails closed on unparseable args; NEW `ragent_tools_core::schema::validate_required_args` schema validation before permissions/dispatch; panicked tool tasks synthesise error results + `ToolCallEnd` (no orphaned `tool_use`); parallel drain continues past watchdog stalls with the real call id; permit failure publishes `ToolCallEnd`; interrupt stops dispatch at the top of the per-call loop; unknown-tool error carries an edit-distance "did you mean" hint; duplicate-Start guard in openai.rs; 1 MiB args cap; shared `normalize_mcp_arguments` for stdio + HTTP MCP transports; text-format tool-call recovery extractor (`session/text_toolcalls.rs`) for models that narrate tool calls as prose (Qwen-style JSON blocks, XML-parameter blocks, bare JSON) with 11-case tests. Edit tools: CRLF line endings preserved through flexible + indent lanes (terminator-aware re-emit) and the cascade lane-2 substitution; BOM stripped for matching and re-prefixed in edit + multi_edit; non-UTF-8 files get a precise error; `byte_offset_to_line` panic hardened with debug asserts at all splice sites. NEW tests: `test_gemini_tool_calls.rs`, `test_edit_utf8_fixes.rs`, `test_text_toolcalls.rs`, Ollama object-args tests. |
