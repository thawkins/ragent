...
impl Content<Vec<Operation>> {
    ...
    ///
    /// This is the entry point used for every page and form-XObject content
    /// stream. The array/dictionary recursion in the object parser is
    /// depth-limited (see `MAX_OBJECT_DEPTH` in `parser`), so a content stream
    /// with pathologically nested operands is rejected here as an ordinary
    /// parse error instead of overflowing the native stack, which used to
    /// abort the whole process with SIGABRT.
