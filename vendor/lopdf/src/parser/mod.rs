...
/// Maximum nesting depth of arrays and dictionaries inside a single PDF
/// object.
///
/// An object such as `[[[[...]]]]` is parsed by mutually recursive
/// object combinators, one native stack frame per level. Valid PDFs nest only
/// a handful of levels deep, so a pathological document (a few thousand
/// opened brackets with no closing ones) would otherwise recurse until the
/// stack guard page is hit. On Linux that raises `SIGSEGV`, which the Rust
/// runtime converts to `SIGABRT`; `catch_unwind` cannot intercept it, so a
/// single malformed file would abort the whole process.
///
/// Exceeding the limit is reported as an ordinary parse error (the object is
/// rejected), never as a panic.
///
/// The counter is incremented once per *parser entry* rather than once per
/// bracket: every nested object descends through `_direct_objects` on the way
/// in and again on the way back out (the `alt` retries the cheap scalar
/// branches before it gives up), so each bracket level spends two units of the
/// budget. The limit is therefore a bound on stack frames, not a bound on
/// bracket nesting.
const MAX_OBJECT_DEPTH: usize = 128;

// Current number of enclosing arrays/dictionaries being parsed.
//
// Thread local because PDF objects are also parsed from worker threads
// (content streams, object streams, and the isolated extraction thread).
// It is always restored by [`DepthGuard`], including on early return via
// `?` or on a caught unwind, so the counter cannot leak between parses.
thread_local! {
    static OBJECT_DEPTH: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}

/// RAII guard which increments `OBJECT_DEPTH` on entry and always
/// decrements it on drop.
struct DepthGuard;

impl DepthGuard {
    /// Enter one array/dictionary level, or return the nom error which
    /// rejects the document as too deeply nested.
    fn enter(
        input: ParserInput<'_>,
    ) -> Result<(ParserInput<'_>, DepthGuard), nom::Err<NomError<'_>>> {
        let too_deep = OBJECT_DEPTH.with(|depth| {
            let current = depth.get();
            if current >= MAX_OBJECT_DEPTH {
                true
            } else {
                depth.set(current + 1);
                false
        ...
        if too_deep {
            // `Error::Fail` is a hard error: nom's `many0`/`fold_many0` will
            // not treat it as "zero items" and retry at the same position.
...
impl Drop for DepthGuard {
    fn drop(&mut self) {
        OBJECT_DEPTH.with(|depth| depth.set(depth.get().saturating_sub(1)));
    }
}
...
fn array(input: ParserInput) -> NomResult<Vec<Object>> {
    let (input, _guard) = DepthGuard::enter(input)?;
...
EI";
    ...
    /// A few thousand unclosed `[` used to recurse through `array` until the
    /// stack guard page was hit; that raises SIGSEGV which the Rust runtime
    /// turns into SIGABRT, aborting the process. It must now be a plain parse
    /// error.
    #[test]
    fn deeply_nested_unclosed_arrays_are_rejected() {
        let input = vec![b'['; MAX_OBJECT_DEPTH * 100];
        assert!(direct_object(test_span(&input)).is_none());
    }

    /// Same for dictionaries: `<<` repeated with unclosed braces.
    #[test]
    fn deeply_nested_unclosed_dictionaries_are_rejected() {
        let input = b"<<".repeat(MAX_OBJECT_DEPTH * 100);
        assert!(direct_object(test_span(&input)).is_none());
    }

    /// Structurally valid nesting just inside the limit still parses.
    ///
    // The limit counts parser entries, not brackets: every nested object is
    // routed through `_direct_objects` on the way down and on the way back up
    // (the `alt` tries `null`/`boolean`/... first), so each bracket level
    // costs two of the budget and the usable bracket nesting is half of
    // MAX_OBJECT_DEPTH.
    #[test]
    fn nesting_within_limit_is_parsed() {
        let depth = 4;
        let mut input = vec![b'['; depth];
        input.extend(vec![b']'; depth]);
        let parsed = direct_object(test_span(&input)).expect("nested array should parse");
        assert!(matches!(parsed, Object::Array(_)));

        // Just beyond half the budget is rejected rather than overflowing.
        let depth = MAX_OBJECT_DEPTH;
        let mut input = vec![b'['; depth];
        input.extend(vec![b']'; depth]);
        assert!(direct_object(test_span(&input)).is_none());
    }

    /// The depth counter must be restored after a rejected parse, otherwise
    /// the budget would leak and later documents would be rejected too.
    #[test]
    fn depth_counter_is_restored_after_rejection() {
        let too_deep = vec![b'['; MAX_OBJECT_DEPTH * 4];
        assert!(direct_object(test_span(&too_deep)).is_none());
        assert_eq!(
            OBJECT_DEPTH.with(std::cell::Cell::get),
            0,
            "depth budget must not leak between parses"
        );
        assert!(direct_object(test_span(b"[1 2 3]")).is_some());
    }
}
