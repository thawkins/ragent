...
| Background task manager        | `bg`                                                    | Partial.`new_agent`/`wait_agents`/`cancel_agent` exist for sub-agents, but no generic background bash task introspection. | High — long-running builds/tests.                 |
