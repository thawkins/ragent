...
use ragent_codeindex::types::{ScanConfig, SymbolFilter, SymbolKind};
use std::fs;
...
fn test_query_symbols_by_visibility() {
    ...
    let pub_symbols = store
        .query_symbols(&SymbolFilter {
            visibility: Some(ragent_codeindex::types::Visibility::Public),
            ..Default::default()
        })
        .unwrap();
...
fn test_references_extracted_from_function_bodies() {
    use ragent_codeindex::types::FileEntry;
