    ...
    while let Some(line) = lines.next() {
        ...
        if lower.starts_with("*** add file:") {
            ...
            while let Some(peek) = lines.peek() {
                ...
                let next = lines
                    .next()
                    .context("Unexpected end of patch in Add file content")?;
            ...
            while let Some(peek) = lines.peek() {
                ...
                if peek_trim.to_lowercase().starts_with("*** move to:") {
                    let next = lines
                        .next()
                        .context("Unexpected end of patch at move-to header")?;
...
fn parse_hunk<'a>(lines: &mut std::iter::Peekable<impl Iterator<Item = &'a str>>) -> Result<Hunk> {
    ...
    while let Some(peek) = lines.peek() {
        ...
        let next = lines
            .next()
            .context("Unexpected end of patch inside hunk")?;
        if next.is_empty() {
        ...
        let first_char = next
            .chars()
            .next()
            .context("Empty hunk line already handled")?;
        match first_char {
