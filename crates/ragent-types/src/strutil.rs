...
mod tests {
    ...
    fn test_truncate_chars_multibyte() {
        // "café" — 'é' is 2 bytes but 1 char
        assert_eq!(truncate_chars("café résumé", 5), "café …");
    }

    #[test]
    fn test_truncate_chars_en_dash() {
        // "A – B" — '–' is 3 bytes, 1 char
    ...
    fn test_truncate_bytes_boundary_adjustment() {
        // "é" is 2 bytes; byte index 3 splits 'é' → adjusted to 2
        assert_eq!(truncate_bytes("café", 3), "caf…");
    }

    #[test]
    fn test_truncate_bytes_en_dash() {
        // "–" is 3 bytes (U+2013); byte index 2 splits it → adjusted to 1
        // "A" is byte 0, " " is byte 1, "–" is bytes 2..5
        // With max_bytes=2, we cut at byte 1 ("A" + space), then back up to 1
