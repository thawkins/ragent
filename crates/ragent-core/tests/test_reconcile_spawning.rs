...
async fn test_reconcile_spawning_members_updates_config() {
    ...
    assert!(loaded2.config.members.iter().any(
        |m| m.name == "auto-1" && m.status == ragent_core::team::config::MemberStatus::Spawning
    ));
    ...
    let member = final_store
        .config
        .member_by_name("auto-1")
        .expect("member present");
    assert!(
        member.session_id.is_some(),
        "reconciled member should have session_id"
    );
}
