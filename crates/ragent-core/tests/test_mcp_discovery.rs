...
fn test_discovered_mcp_server_npm_global_source() {
    let server = DiscoveredMcpServer {
        ...
        executable: PathBuf::from("/usr/local/lib/node_modules/@modelcontextprotocol/server-github/dist/index.js"),
        args: vec![],
        env: HashMap::new(),
...
fn test_discovered_mcp_server_registry_source() {
    ...
    let config = server.to_config();
    assert_eq!(config.command, Some("/home/user/bin/custom-mcp".to_string()));
