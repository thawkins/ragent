...
fn test_substitute_arguments_full_string() {
    let result = substitute_args("Run: $ARGUMENTS", "fix the bug", "sess-1", Path::new("/sk"));
...
fn test_substitute_arguments_empty() {
    let result = substitute_args("Run: $ARGUMENTS end", "", "sess-1", Path::new("/sk"));
...
fn test_substitute_positional_one() {
    ...
    let result = substitute_args("Second: $1", "alpha beta", "sess-1", Path::new("/sk"));
    assert_eq!(result, "Second: beta");
}

#[test]
fn test_substitute_positional_two() {
    let result = substitute_args("Third: $2", "alpha beta gamma", "sess-1", Path::new("/sk"));
    assert_eq!(result, "Third: gamma");
}

#[test]
fn test_substitute_positional_out_of_bounds() {
    let result = substitute_args("Missing: $9", "only-one", "sess-1", Path::new("/sk"));
    assert_eq!(result, "Missing: ");
}
...
fn test_substitute_preserves_unrecognized_braces() {
    let result = substitute_args("Path: ${HOME}/data", "", "sess-1", Path::new("/sk"));
...
fn test_substitute_empty_body() {
    let result = substitute_args("", "ignored", "sess-1", Path::new("/sk"));
    assert_eq!(result, "");
}
