...
async fn test_team_create_applies_blueprint_readme_tasks_and_spawn() {
    ...
    let _out = create
        .execute(
            serde_json::json!({"name":"bp-sample","project_local":true,"blueprint":"bp1"}),
            &lead_ctx,
        )
        .await
        .unwrap();
    ...
    assert!(
        titles.iter().any(|t| t.contains("Seed Task")),
        "Seed Task should exist"
    );
    assert!(
        titles.iter().any(|t| t.contains("Tool Created")),
        "Tool-created task should exist"
    );

    // Assert spawn created teammates (auto-1 etc.)
    let members: Vec<String> = loaded
        .config
        .members
        .iter()
        .map(|m| m.name.clone())
        .collect();
    assert!(
        members.iter().any(|n| n == "auto-1"),
        "auto-1 should be present"
    );
    assert!(
        members.iter().any(|n| n == "auto-2"),
        "auto-2 should be present"
    );
