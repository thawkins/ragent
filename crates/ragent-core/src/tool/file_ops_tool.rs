...
impl Tool for FileOpsTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let concurrency = input["concurrency"].as_i64().map(|n| n as usize).unwrap_or_else(|| num_cpus::get());
        let dry_run = input["dry_run"].as_bool().unwrap_or(false);

        let mut pairs = Vec::with_capacity(edits_arr.len());
        for e in edits_arr {
            let path = e["path"].as_str().context("edit.path missing or not string")?;
            let content = e["content"].as_str().context("edit.content missing or not string")?;
            let resolved = if std::path::Path::new(path).is_absolute() { Path::new(path).to_path_buf() } else { ctx.working_dir.join(path) };
            pairs.push((resolved, content.to_string()));
        }

        let res: CommitResult = apply_batch_edits(pairs, concurrency, dry_run).await?;

        let summary = format!("applied={} conflicts={} errors={}", res.applied.len(), res.conflicts.len(), res.errors.len());
        Ok(ToolOutput { content: summary, metadata: Some(json!({"applied": res.applied.len(), "conflicts": res.conflicts.len(), "errors": res.errors.len()})) })
    }
}
