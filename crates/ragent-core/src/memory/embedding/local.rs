...
///
/// ```no_run
/// use ragent_core::memory::embedding::{EmbeddingProvider, LocalEmbeddingProvider};
///
...
impl LocalEmbeddingProvider {
    ...
    fn download_file(filename: &str, dest: &PathBuf) -> Result<()> {
        ...
        // Use a blocking Tokio runtime since reqwest is async-only.
    ...
    fn encode(&self, text: &str) -> Result<Vec<f32>> {
        ...
        // Get the tokenizer from the inner state (tokenizer is immutable after init).
        ...
        // Tokenise input (tokenizer is immutable, so can use from behind &ref).
        ...
        // Build input tensors as ndarray arrays.
        ...
        // Get a mutable reference to the session to run inference.
        ...
        // Extract as dynamic array view.
        let output_view: ndarray::ArrayViewD<f32> = first_output
            .try_extract_array::<f32>()
            .map_err(|e| anyhow::anyhow!("Failed to extract output tensor: {e}"))?;

        // Mean pooling: weighted average across the sequence dimension.
        // Shape: [1, seq_len, dim] → [dim]
        ...
        if shape.len() == 3 && shape[0] == 1 && shape[2] == dim {
            ...
            for s in 0..seq_len_out {
                // Use attention_mask for pooling if available.
                // For position s, use the corresponding mask value.
            ...
            // Some models output [1, dim] directly (already pooled).
            // Extract first row from dynamic array.
...
impl EmbeddingProvider for LocalEmbeddingProvider {
    fn embed(&self, text: &str) -> Result<Vec<f32>> {
        match self.encode(text) {
            Ok(vec) => Ok(vec),
            Err(e) => {
                // Mark the provider as failed so we don't retry on every call.
