...
async fn get_providers(State(state): State<AppState>) -> impl IntoResponse {
    ...
    Json(serde_json::to_value(provider_ids).unwrap_or_default())
}
...
async fn create_session(
    ...
    match state.storage.create_session(&id, &body.directory) {
        Ok(()) => {
            state
                .event_bus
            ...
            (
                StatusCode::CREATED,
                Json(serde_json::json!({ "id": id, "directory": body.directory })),
            )
                .into_response()
        }
        Err(e) => (
...
async fn get_messages(
    ...
    match state.storage.get_messages(&id) {
        Ok(messages) => {
            (StatusCode::OK, Json(serde_json::to_value(messages).unwrap())).into_response()
        }
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": e.to_string() })),
...
async fn send_message(
    ...
    tokio::spawn(async move {
        ...
        let agent = agent::resolve_agent(&cfg.default_agent, &cfg).unwrap_or_else(|_| {
            AgentInfo::new("general", "General-purpose agent")
        });
        drop(cfg);
        if let Err(e) = processor.process_message(&session_id, &content, &agent).await {
            tracing::error!(session_id = %session_id, error = %e, "Failed to process message");
        }
    });
...
async fn reply_permission(
    ...
    let allowed = body.decision != "deny";
...
fn event_matches_session(event: &Event, session_id: &str) -> bool {
    match event {
        ...
        | Event::MessageStart { session_id: sid, .. }
        | Event::TextDelta { session_id: sid, .. }
        | Event::ReasoningDelta { session_id: sid, .. }
        | Event::ToolCallStart { session_id: sid, .. }
        | Event::ToolCallEnd { session_id: sid, .. }
        | Event::MessageEnd { session_id: sid, .. }
        | Event::PermissionRequested { session_id: sid, .. }
        | Event::PermissionReplied { session_id: sid, .. }
        | Event::AgentSwitched { session_id: sid, .. }
        | Event::AgentError { session_id: sid, .. }
        | Event::TokenUsage { session_id: sid, .. } => sid == session_id,
