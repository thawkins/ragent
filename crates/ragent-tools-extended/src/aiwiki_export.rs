    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        match format {
            "single_markdown" => {
                ...
                match ragent_aiwiki::export_single_markdown(&wiki, &output_path).await {
                    Ok(count) => Ok(ToolOutput {
                        content: format!(
                            "## Export Complete ✅\n\n\
                                 Exported {} pages to:\n\
                                 `{}`\n\n\
            ...
            "obsidian" => {
                let output_dir = input["output_path"]
                    .as_str()
                    .map(|p| ctx.working_dir.join(p))
                    .unwrap_or_else(|| ctx.working_dir.join("aiwiki_obsidian"));

                match ragent_aiwiki::export_obsidian_vault(&wiki, &output_dir).await {
                    Ok(count) => Ok(ToolOutput {
                        content: format!(
