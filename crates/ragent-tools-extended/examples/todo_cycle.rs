...
impl StorageBackend for DemoStorage {
    ...
    fn list_memories(&self, _p: &str, _l: usize) -> anyhow::Result<Vec<MemoryRow>> {
        Ok(Vec::new())
    }
    fn store_memory_embedding(&self, _id: i64, _blob: &[u8]) -> anyhow::Result<bool> {
        Ok(false)
    }
...
async fn main() {
    ...
    let out = read_tool
        .execute(json!({}), &make_ctx(storage.clone()))
        .await
        .unwrap();
    ...
    let out = read_tool
        .execute(json!({"status":"all"}), &make_ctx(storage.clone()))
        .await
        .unwrap();
    print_output(&out.content);
    ...
    let out = read_tool
        .execute(json!({"status":"done"}), &make_ctx(storage.clone()))
        .await
        .unwrap();
    print_output(&out.content);
    ...
    let out = write_tool
        .execute(json!({"action":"clear"}), &make_ctx(storage.clone()))
        .await
        .unwrap();
    print_output(&out.content);
