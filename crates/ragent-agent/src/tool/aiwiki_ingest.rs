...
impl Tool for AiwikiIngestTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let options = if let Some(subdir) = subdirectory {
            ragent_aiwiki::IngestOptions::move_file()
                .with_move(move_file)
                .with_subdirectory(subdir)
        } else {
            ragent_aiwiki::IngestOptions::move_file().with_move(move_file)
        };
        ...
        let result = if let Some(path_str) = input["path"].as_str() {
            ...
            if metadata.is_file() {
                // Ingest single file
                let result = ragent_aiwiki::ingest_file(&wiki, &path, options).await?;
                format_ingestion_result(&result)
            } else if metadata.is_dir() {
                // Ingest directory
                let results = ragent_aiwiki::scan_directory(&wiki, &path, options, true).await?;
            ...
            // Scan raw/ directory
            let options = ragent_aiwiki::IngestOptions::default();
            let results = ragent_aiwiki::ingest_raw_directory(&wiki, options).await?;
...
fn format_ingestion_result(result: &ragent_aiwiki::IngestionResult) -> ToolOutput {
    let content = format!(
...
fn format_raw_scan_result(results: &[ragent_aiwiki::IngestionResult]) -> ToolOutput {
    if results.is_empty() {
        return ToolOutput {
