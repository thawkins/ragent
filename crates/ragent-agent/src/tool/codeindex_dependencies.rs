...
impl Tool for CodeIndexDependenciesTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let direction = match input["direction"].as_str() {
            Some("dependents") => ragent_codeindex::types::DepDirection::Dependents,
        ...
        if deps.is_empty() {
            let dir_label = match direction {
                ragent_codeindex::types::DepDirection::Imports => "imports",
                ragent_codeindex::types::DepDirection::Dependents => "dependents",
            };
