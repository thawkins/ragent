...
impl Tool for AiwikiStatusTool {
    ...
    async fn execute(&self, _input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let auto_sync_str = match wiki.config.sync_mode {
            ragent_aiwiki::SyncMode::Manual => "Disabled (manual)",
            ragent_aiwiki::SyncMode::OnStartup => "On startup",
            ragent_aiwiki::SyncMode::Realtime => "Realtime",
        };
...
async fn gather_stats(wiki: &ragent_aiwiki::Aiwiki) -> WikiStats {
    ...
    let needs_sync = ragent_aiwiki::needs_sync(wiki).await.unwrap_or(false);

    // Get sync preview for pending count
    let pending_sync = if let Ok(preview) = ragent_aiwiki::preview_sync(wiki).await {
