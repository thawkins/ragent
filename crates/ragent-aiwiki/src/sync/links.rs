...
fn resolve_internal_link(wiki_dir: &Path, source: &Path, target: &str) -> PathBuf {
    ...
    if let Some(source_dir) = source.parent() {
        let resolved = normalize_path(source_dir.join(target_path));
        if resolved.exists() {
            return resolved;
        }
    ...
    normalize_path(wiki_dir.join(target_path))
}

/// Normalize a path lexically without requiring it to exist on disk.
