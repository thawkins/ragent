...
impl SourceWatcher {
    ...
    pub fn new(
        ...
        for source in sources {
            ...
            let watch_path = if source_path.exists() {
                ...
                source_path.clone()
            } else if let Some(existing_parent) = nearest_existing_dir(&source_path, &root) {
                debug!(
                    "source '{}' does not exist yet; watching parent '{}'",
                    source_path.display(),
            ...
            watcher
                ...
                .with_context(|| format!("cannot watch source: {}", watch_path.display()))?;
...
fn nearest_existing_dir(path: &Path, root: &Path) -> Option<PathBuf> {
